...
def impliedVolatility(className, args, callPrice=None, putPrice=None, high=500.0, low=0.0):
	...
	for i in range(10000):	# To avoid infinite loops
		...
		if abs(estimate - target) < 0.000001: 
			break
...
'''
   European Options Pricing Formula via Black-Scholes Model
 * @s: underlying asset price. e.g. ETHUSD, BTCUSD index
 * @k: strike price
 * @r: risk-free interest rate
 * @t: maturity to expire in year
 * @v: volatility, our quote volatility or implied volatility
 * @cp: call or put, +1/-1 ~ call/put
'''
import math
import numpy as np
...
def pdf(x):
    return math.exp(-math.pow(x, 2) / 2.0) / math.sqrt(2 * np.pi)


def calc_d1(s, k, r, t, v): 
    return (math.log(s / k) + (r + 0.5 * math.pow(v, 2)) * t) / (v * math.sqrt(t))
          

def calc_price(s, k, r, t, v, cp):
    if v <= 0:
        return max(0, cp * (s - k))

    d1 = calc_d1(s, k, r, t, v)
    d2 = d1 - v * math.sqrt(t)

    return cp * (s * cdf(cp * d1) - k * cdf(cp * d2) * math.exp(-r * t))



def calc_delta(s, k, r, t, v, cp):
    if v <= 0:
        return 0
...
def calc_theta(s, k, r, t, v, cp):
    return calc_original_theta(s, k, r, t, v, cp) / 365



def calc_original_theta(s, k, r, t, v, cp):
    if v <= 0:
        return 0
    ...
    return (-0.5 * s * pdf(d1) * v / math.sqrt(t) - cp * r * k * math.exp(-r * t) * cdf(cp * d2))


def calc_vega(s, k, r, t, v, cp):
    return calc_original_vega(s, k, r, t, v, cp) / 100


def calc_original_vega(s, k, r, t, v, cp):
    if v <= 0:
        return 0
...
def calc_greeks(s, k, r, t, v, cp):
    delta = calc_delta(s, k, r, t, v, cp)
    gamma = calc_gamma(s, k, r, t, v, cp)
    theta = calc_theta(s, k, r, t, v, cp)
    vega = calc_vega(s, k, r, t, v, cp)

    return [delta, gamma, theta, vega]



def calc_implv(price, s, k, r, t, cp):
    if price <= 0 :
        return 0

    meet = False
    if cp == 1 and price > (s - k) * math.exp(-r * t): 
        meet = True
    elif cp == -1 and price > (k * math.exp(-r * t) - s):
        meet = True
    ...
    i = 0
    while i < 200: 
        p = calc_price(s, k, r, t, v, cp)
        vega = calc_original_vega(s, k, r, t, v, cp)
