...
def on_response_modify_order_ready(context, data):
    ...
    with context.lock:
        if not error:
            ...
            context.logger.error(f"Modify order error:{metadata['metadata']['error_code_msg']}")
...
def update_order(context, data):
    ...
    if status == OrderStatus.Filled or status == OrderStatus.Cancelled:
        ...
        if key in context.hedge_positions[symbol]:
            ...
            context.hedge_positions[symbol][key] = {'qty':target_qty,'side':target_side}
    ...
    for basket in context.baskets:
        ...
        if len(gammas)!=context.contract_num:
            ...
            try:
                ...
                target_func = np.append(gs + vs,- abs(group_gamma) - abs(group_vega)/10000)
...
def hedge_execution(context,group,data_dict):
    with context.lock:
        ...
        for sym in in_syms:
            ...
            note = {'send_new':False, 'group':group,'target_qty':data_dict[sym]['target_qty'],'target_side':data_dict[sym]['target_side']}
            ...
            if open_order['note']['aggresive'] == 'maker':
                ...
                if side.lower()!=order_side.lower():
                    context.cancel_order(f'Deribit|{sym}|option',sent_order_id,note=note) 
                        ...
                        context.modify_order(f'Deribit|{sym}|option', sent_order_id, best_price, abs(diff))
            else:
                context.cancel_order(f'Deribit|{sym}|option',sent_order_id,note=note) 
